...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#actName").keydown(function(event){
			if (event.keyCode == 13){
				$.ajax({
					url : "ssm/clue/getActivityListRelationNotClueId",
					data : {
					...
					success : function(data){
						...
						$.each(data, function(i,n){
							html += '<tr>';
							html += '<td><input type="checkbox" name="opt" value="'+ n.id +'" /></td>';
							html += '<td>'+ n.name +'</td>';
		...
		$("#clear").click(clearRealtion);

		$("#bindBtn").click(function(){
			var $opt = $(":checkbox[name='opt']:checked");
			if ($opt.length == 0){
				alert("选项不能为空");
			} else{
				var param = "clueId=${requestScope.clue.id}&";
				for (var i = 0; i < $opt.length; i++) {
					param += "activityId="+ $($opt[i]).val();
					if (i < $opt.length-1){
						param += "&";
					}
				}
				$.ajax({
					url : "ssm/clue/bindRelation",
					data : param,
					type : "post",
					dataType : "json",
					success : function(data){
	...
	function showActivityList(){
		$.ajax({
			url : "ssm/activity/showActivityToClueDetail",
			data : {
				"clueId": "${requestScope.clue.id}"
			},
			type : "get",
	...
	function clearRealtion(){
		$("#actName").val("");
		$("#activityRelationBody").html("");
	}

	function allOptAndNotOpt(){
		$("#allOpt").click(function(){
			$(":checkbox[name='opt']").prop("checked", this.checked);
		})

		$("#activityRelationBody").on("click", $(":checkbox[name='opt']"), function(){
			$("#allOpt").prop("checked", $(":checkbox[name='opt']").length == $(":checkbox[name='opt']:checked").length);
		})
	}
					...
					<table id="activityTable" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
						<thead>
							<tr style="color: #B3B3B3;">
								<td><input type="checkbox" id="allOpt"/></td>
								<td>名称</td>
								<td>开始日期</td>
								<td>结束日期</td>
								<td>所有者</td>
...
<%--							</tr>--%>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal" id="clear">取消</button>
					<button type="button" class="btn btn-primary" data-dismiss="modal" id="bindBtn">关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='ssm/clue/getClueConvertPage?id=${requestScope.clue.id}'"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
...
<%--						</tr>--%>
			...
			<div>
				<a href="javascript:void(0);" data-toggle="modal" data-target="#bundModal" id="relationBtn" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
